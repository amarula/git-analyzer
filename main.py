...
def main():
    ...
    if args.config_file:
        ...
        if config_data:
            ...
            print(
                f"Warning: Failed to load configuration from {
                    args.config_file}."
                ' Proceeding with command-line arguments or prompts.',
            )

    # If config file was NOT successfully loaded, or not provided, then use
    # CLI args
    ...
    if not repo_urls:
        ...
        repo_urls = [url.strip()
                     for url in repo_urls_input.split(',') if url.strip()]
    ...
    if months_back > 1:
        ...
        # sentinel: use legacy behaviour
        calendar_months = [(None, None, None)]
